    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center justify-between border-b border-gray-200 p-4 flex-shrink-0">
        <div className="flex items-center gap-4">
          <h2 className="text-xl font-semibold text-gray-800">{sheetName}</h2>
          <button
          ...
          <table className="border-collapse" style={{ tableLayout: 'fixed', width: `${calculateTableWidth()}px` }}>
            <thead className="sticky top-0 z-10">
              <tr className="bg-gray-100">
                <th className="px-4 py-3 text-center text-xs font-semibold text-gray-600 uppercase border-r border-b border-gray-300" style={{ width: 60 }}>
                  #
                </th>
                {fields.map((field) => (
                  <th
                    key={field.id}
                    className="relative px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase border-r border-b border-gray-300"
              ...
              {rows.length === 0 ? (
                ...
                rows.map((row, rowIndex) => (
                  <tr key={`${row.uuid}-${row.user_id || 'admin'}`} className="hover:bg-gray-50">
                    <td className="text-center text-sm text-gray-600 border-r border-b border-gray-200 bg-gray-50 p-0" style={{ width: 60 }}>
